...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    function jobOccurrences(anchor: Date, frequency: string): Date[] {
      ...
      if (step !== null) {
        // Remonte jusqu'avant fromDate, puis avance jusqu'à toDate
        ...
        // Fréquence inconnue : affiche juste l'ancre si dans la plage
    ...
    for (const j of jobs) {
      ...
      if (!anchor) continue
      const occurrences = jobOccurrences(anchor, j.frequency)
  ...
  try {
    const rows = await db
      .select({ id: actionLogs.id, createdAt: actionLogs.createdAt, type: actionLogs.type, status: actionLogs.status, slug: agentInstances.agentSlug })
      .from(actionLogs)
  ...
  try {
    ...
    if (creds) {
      ...
      for (const g of gevents) {
        ...
        // Déduplique avec les events DB ayant ce googleEventId
  ...
  return NextResponse.json({ events: all, googleConnected })
}
