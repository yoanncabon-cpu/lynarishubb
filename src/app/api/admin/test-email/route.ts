...
export async function GET() {
  // Auth guard — réservé aux admins Lynaris
  ...
  const to = adminEmails[0] ?? user.email ?? "yoanncabon@gmail.com"
  ...
  const html = emailLayout(`
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="width:100%;margin-bottom:24px">
      ...
      <tr>
        <td style="padding:10px 0;font-size:13px;color:rgba(250,250,250,0.45);font-family:system-ui">Déclenché par</td>
        <td style="padding:10px 0;font-size:13px;color:#34D399;font-weight:600;font-family:system-ui">${user.email}</td>
      </tr>
    </table>
  `)
  ...
  return NextResponse.json({ success: result.success, to, from: gmailUser, error: result.error })
}
